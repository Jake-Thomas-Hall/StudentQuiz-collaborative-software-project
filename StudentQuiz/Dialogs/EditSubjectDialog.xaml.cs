...
namespace StudentQuiz.Dialogs
{
    public sealed partial class EditSubjectDialog : ContentDialog
    {
        ...
        public EditSubjectDialog(Subject subject)
        {
            Subject = subject;
        ...
        private async void ContentDialog_PrimaryButtonClick(ContentDialog sender, ContentDialogButtonClickEventArgs args)
        {
            ...
            try
            {
                ...
                {
                    await SubjectDataController.UpdateSubject(Subject);
                });
                deferral.Complete();
            }
            catch(DbException)
            {
                args.Cancel = true;
                SubjectDuplicateInfobar.Margin = new Thickness(0, 0, 0, 16);
                ErrorInfobar.Message = $"Could not update subject to '{Subject.Name}', this active subject already exists";
                ErrorInfobar.IsOpen = true;
                deferral.Complete();
                return;
            }
